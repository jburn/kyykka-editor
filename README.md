...
mark the project as modified. A different source video starts fresh.
...
**Open project** (Ctrl+O) to save and load `.kyykka` files. You can also drop a
single `.kyykka` file onto the window to open it. Dropping a video (`.mp4`,
`.mov`, `.mkv`, `.avi`, or `.m4v`) opens Match details for a new project with
that video selected. Unsaved changes still prompt you to save, discard, or
cancel before replacing the current project. File drops are disabled during
export. The title bar shows
